...
const DataStewardDashboard = () => {
  const cards = [
    { title: 'Total Datasets', value: '5', icon: <FiList />, color: 'text-blue-500', bg: 'bg-blue-50' },
    { title: 'Active Rules', value: '3', icon: <FiShield />, color: 'text-amber-500', bg: 'bg-amber-50' },
    { title: 'Lineage Mapping', value: '4', icon: <FiShare2 />, color: 'text-purple-500', bg: 'bg-purple-50' },
  ];

  return (
    <MainLayout>
      <h1 className="text-2xl md:text-3xl font-black text-slate-900 mb-8">Data Steward Dashboard</h1>
      
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
        {cards.map((card, i) => (
          <div key={i} className="bg-white p-8 rounded-3xl border border-slate-100 shadow-sm flex items-center gap-6">
            <div className={`w-14 h-14 rounded-2xl ${card.bg} ${card.color} flex items-center justify-center text-2xl`}>
              {card.icon}
            </div>
            <div>
