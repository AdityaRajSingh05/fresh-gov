...
//NEW CODE:- MOCK WITH NEW DATASET REGISTEED
...
// --- Custom Node Component (STAYING SAME AS YOUR ORIGINAL) ---
const CustomNode = ({ data }) => (
    <div className="px-4 py-2 shadow-md rounded-md bg-white border-2 border-slate-200 min-w-[150px]">
        <Handle type="target" position={Position.Left} className="w-2 h-2 !bg-blue-500" />
        <div className="flex flex-col">
...
export default function LineagePage() {
    ...
    const [edges, setEdges] = useState([]);
    const [selectedDataset, setSelectedDataset] = useState('');
    const [allDatasets, setAllDatasets] = useState([]); // Dynamic data from dbs.json
    const [sidebarOpen, setSidebarOpen] = useState(true);

    // --- NEW: Fetch live datasets from mock server ---
    useEffect(() => {
    ...
    // --- NEW: Dynamic Logic with UPSTREAM/DOWNSTREAM labels ---
    const handleDatasetChange = (e) => {
        ...
        const ds = allDatasets.find(item => item.id.toString() === datasetId.toString());
        ...
        if (ds) {
            const newNodes = [
                { 
                    id: '1', 
                    type: 'custom', 
                    data: { label: ds.upstream || 'Source System', type: 'UPSTREAM' }, 
                    position: { x: 0, y: 100 } 
                },
                { 
                    id: '2', 
                    type: 'custom', 
                    data: { label: ds.name || ds.datasetName, type: 'DATASET' }, 
                    position: { x: 300, y: 100 } 
                },
                { 
                    id: '3', 
                    type: 'custom', 
                    data: { label: ds.downstream || 'Target System', type: 'DOWNSTREAM' }, 
