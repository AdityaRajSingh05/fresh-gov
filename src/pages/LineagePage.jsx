...
export default function LineagePage() {
    ...
    // --- UPDATED: Fetch Datasets and filter out non-existent ones ---
    useEffect(() => {
        const loadDatasets = () => {
            getDataset().then(data => {
                setDatasets(data);
                
                // If the currently selected dataset was deleted, clear the graph
                if (datasetId && !data.find(ds => ds.id.toString() === datasetId.toString())) {
                    setDatasetId('');
                    setNodes([]);
                    setEdges([]);
                }
        ...
        // Optional: Re-fetch when window regains focus to catch deletions from other tabs
        window.addEventListener('focus', loadDatasets);
        return () => window.removeEventListener('focus', loadDatasets);
    }, [datasetId]);
        ...
        getLineage(datasetId).then(data => {
            ...
            const fetchedNodes = data.nodes.map((node) => ({
                ...
                data: {
                    label: node.data.label,
                    type: node.data.type,
                    isMain: node.id.toString() === datasetId.toString()
                },
                position: node.position,
            }));
            ...
            // Handle cases where getLineage fails because dataset was deleted
            setNodes([]);
            setEdges([]);
        });
    }, [datasetId]);
                ...
                <div className="p-8 pb-0">
                    ...
                    <div className="mt-6 max-w-sm mb-4">
                        <select
                            value={datasetId}
                            onChange={handleDatasetChange}
                            className="w-full p-3 bg-white border border-slate-200 rounded-lg shadow-sm focus:ring-2 focus:ring-blue-500 text-slate-700 cursor-pointer"
                        >
                            <option value="">-- Select Dataset --</option>
                            {datasets.map(ds => (
                                <option key={ds.id} value={ds.id}>{ds.name}</option>
                ...
                <div className="flex-1 bg-white border-t border-slate-200 relative w-full h-full">
                    {datasetId && nodes.length > 0 ? (
                        ...
                        <div className="flex items-center justify-center h-full text-slate-400 italic">
                            <p>Please select a dataset to view lineage</p>
                        </div>
                    )}
                </div>
            </main>
